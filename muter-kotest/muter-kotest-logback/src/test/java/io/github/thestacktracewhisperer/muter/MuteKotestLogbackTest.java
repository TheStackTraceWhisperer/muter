...
/**
 * Integration tests for the Logback implementation of the Kotest muter listener.
 */
class MuteKotestLogbackTest {
    ...
    void missingLogbackBindingFailsFast() {
        ...
        assertEquals(
                "muter-kotest-logback requires Logback Classic on the classpath; found: java.lang.Object",
                ex.getMessage());
    }
    ...
    void nullLoggerFactoryFailsFast() {
        ...
        IllegalStateException ex = assertThrows(IllegalStateException.class, () -> muter.mute(new Class<?>[0]));
        assertEquals(
                "muter-kotest-logback requires Logback Classic on the classpath; found: null",
                ex.getMessage());
    }

    // ---------- MuteKotestListener + LogbackMuter integration ----------

    @Test
    @DisplayName("MuteKotestListener with LogbackMuter mutes root and restores for @Mute spec")
    void listenerMutesRootLoggerAndRestores() {
        ROOT.setLevel(Level.INFO);
        MuteKotestListener listener = new MuteKotestListener(List.of(new LogbackMuter()));
    ...
    @Test
    @DisplayName("MuteKotestListener with LogbackMuter mutes specific class logger")
